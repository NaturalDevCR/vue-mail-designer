<template>
  <section class="vmd-modal-box vmd-ai-template-box vmd-ai-template-panel" role="region" :aria-label="t('aiTemplates.title')">
    <header class="vmd-ai-template-header">
      <div>
        <h2>{{ t('aiTemplates.title') }}</h2>
        <p>{{ t('aiTemplates.subtitle') }}</p>
      </div>
      <button v-if="!embedded" type="button" class="vmd-btn vmd-btn--icon" :title="t('common.close')" @click="close">×</button>
    </header>
    ...
    <div v-if="proposals.length === 0" class="vmd-ai-template-form">
      ...
      <div class="vmd-ai-template-actions">
        <button v-if="!embedded" type="button" class="vmd-btn" :disabled="loading" @click="close">{{ t('common.close') }}</button>
...
const emit = defineEmits<{ error: [payload: AiTemplateErrorPayload]; close: [] }>()
