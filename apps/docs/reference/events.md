...
| `ai-templates-error` | `AiTemplateErrorPayload` | When dynamic context resolution, the host provider callback, or returned proposal validation fails. `operation` is `'context' | 'generate' | 'validate'`. |
...
For the full request/response contract and provider security guidance, see [AI template generation](/guide/ai-template-generation).
